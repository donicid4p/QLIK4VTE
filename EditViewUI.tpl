		...
		<div {if $OLD_STYLE eq true}style="float:left; padding-top:5px;"{/if}>
			...
			{if $smarty.request.enable_editoptions eq 'yes'}
				...
				{if $fldname|in_array:$editoptionsfieldnames}
					<div class="editoptions" fieldname="other_{$fldname|@vtlib_purify}" optionstype="referencefieldnames" style="float:right;"></div>{* crmv@211287 *}
				{/if}
			{/if}
...
{elseif $uitype eq 11 || $uitype eq 1 || $uitype eq 13 || $uitype eq 7 || $uitype eq 9 || $uitype eq 1112 || $uitype eq 1013 || $uitype eq 1014}
	...
	{* qlikiframe - campi credenziali modificabili solo ad amministratori *}
	{elseif ($fldname eq 'qlikiframe_login_qlik' || $fldname eq 'qlikiframe_pwd_qlik') && $isadmin neq 1}
...
{elseif $uitype eq 33}
	...
	{else}
		<div class="{$DIVCLASS}" {if $secondvalue.picklist_display neq ''}style="display:{$secondvalue.picklist_display}"{/if}>
			...
			{foreach item=arr from=$fldvalue}
				<option value="{$arr[1]}" {$arr[2]} title="{$arr[0]}">{$arr[0]}</option> {* crmv@246589 *}
			{/foreach}
			</select>
		</div>
	{/if}
			...
			<td width="{if $OLD_STYLE eq true}70{else}80{/if}%" style="position:relative">	{* crmv@57221 *} {* crmv@98866 *}
				...
				<div {if $fld_displayvalue|trim eq ''}class="{$DIVCLASS}"{else}class="dvtCellInfoOff"{/if} id="{$assign_user_div}" style="position:relative; {$style_user}">
					...
					{if $fld_displayvalue|trim eq ''}
						...
						{assign var=fld_style value='class="detailedViewTextBox"'}
					{/if}
					<input id="{$fldname}_display" name="{$fldname}_display" type="text" value="{$fld_displayvalue|escape}" {$fld_style} autocomplete="off">  {* crmv@113776 crmv@234883 *}
					<script type="text/javascript">
						initAutocompleteUG('Users','{$fldname}','{$fldname}_display',{$JSON->encode($fldvalue)});	{* crmv@31950 crmv@234883 *}
					</script>
					<div class="dvtCellInfoImgRx">
				...
				{if $secondvalue neq ''}
					...
					<div {if $fld_displaysecondvalue|trim eq ''}class="{$DIVCLASS}"{else}class="dvtCellInfoOff"{/if} id="{$assign_team_div}" style="position:relative; {$style_group}">
						...
						{if $fld_displaysecondvalue|trim eq ''}
							...
							{assign var=fld_style value='class="detailedViewTextBox"'}
						{/if}
						<input id="{$fldgroupname}_display" name="{$fldgroupname}_display" type="text" value="{$fld_displaysecondvalue|escape}" {$fld_style} autocomplete="off">  {* crmv@113776 crmv@234883 *}
						<script type="text/javascript">
							initAutocompleteUG('Groups','{$fldgroupname}','{$fldgroupname}_display',{$JSON->encode($secondvalue)});	{* crmv@31950 crmv@234883 *}
						</script>
						<div class="dvtCellInfoImgRx">
				...
				{if $thirdvalue.enable_editoptions}
					<div class="{$DIVCLASS}" id="{$assign_other_div}" style="position:relative; {$style_other}">
						...
						<i style="display:{$thirdvalue.advanced_field_assignment_display}" id="advanced_field_assignment_button_{$fldname}" class="vteicon md-link" title="{'LBL_PM_ADVANCED_FIELD_ASSIGNMENT'|getTranslatedString:'Settings'}" onClick="ActionTaskScript.openAdvancedFieldAssignment('{$smarty.request.id|@vtlib_purify|escape:'quotes'}','{$smarty.request.elementid|@vtlib_purify|escape:'quotes'}','{$smarty.request.action_id|@vtlib_purify|escape:'quotes'}','{$fldname}','{$MODULE}','popup',true)" storage="db">more_horiz</i>{* crmv@211287 *}
					</div>
				{/if}
				{* crmv@92272e *}
			</td>
			...
			<tr id="container_{$sdk_params_field}" style="display: none">
				...
				<td colspan="3">
					...
					<div class="dvtCellInfo">
						<input type="input" name="{$sdk_params_field}" id="{$sdk_params_field}" value="{$smarty.request.$sdk_params_field|@vtlib_purify}" class="detailedViewTextBox">{* crmv@211287 *}
					</div>
				</td>
			</tr>
		{/if}
...
{elseif $uitype eq 52 || $uitype eq 77 || $uitype eq 54}	{* crmv@101683 *}
	...
	{if $secondvalue.enable_editoptions eq 'yes'}
		...
		<i style="display:{$secondvalue.advanced_field_assignment_display}" id="advanced_field_assignment_button_{$fldname}" class="vteicon md-link" title="{'LBL_PM_ADVANCED_FIELD_ASSIGNMENT'|getTranslatedString:'Settings'}" onClick="ActionTaskScript.openAdvancedFieldAssignment('{$smarty.request.id|@vtlib_purify|escape:'quotes'}','{$smarty.request.elementid|@vtlib_purify|escape:'quotes'}','{$smarty.request.action_id|@vtlib_purify|escape:'quotes'}','{$fldname}','{$MODULE}','popup',true)" storage="db">more_horiz</i> {* crmv@106856 *}{* crmv@211287 *}
		</td>
		</tr>
		</table>
	{/if}
...
{elseif $uitype eq 23 || $uitype eq 5 || $uitype eq 6}
	...
	<div class="{$DIVCLASS}">
		...
		{* crmv@82419 crmv@100585 *}
			<input name="{$fldname}" class="detailedViewTextBox input-inline vcenter" tabindex="{$vt_tab}" id="jscal_field_{$fldname}" type="text" maxlength="10" value="{$date_val}" {if $fromlink eq 'qcreate' && $fldname eq 'date_start'}onchange="parent.calDuedatetimeQC(this.form,'date');"{/if}  {if $smarty.request.enable_editoptions eq 'yes'}style="display:none"{/if}>	{* //crmv@31315 *}
			<i class="vteicon md-link vcenter" id="jscal_trigger_{$fldname}" {if $smarty.request.enable_editoptions eq 'yes'}style="display:none"{/if}>events</i>
		</td>
		{* crmv@22583 *}
		...
		<tr>
			<td colspan="4" nowrap>
				{* crmv@239717 *}
				{assign var="editoptionsfieldnames" value='|'|explode:$smarty.request.editoptionsfieldnames}
				<i id="tablefields_seq_btn_{$fldname}_opt_num" class="vteicon md-link" style="float:right; display:none;" onclick="ActionUpdateScript.insertTableFieldValue(this,'{$fldname}_opt_num','seq')">input</i>
				<input type="text" id="tablefields_seq_{$fldname}_opt_num" size="2" style="padding-left:5px; float:right; display:none;">
				<div class="tablefields_options" id="tablefields_options_{$fldname}_opt_num" style="float:right; display:none;">
					<select class="populateField" onchange="ActionUpdateScript.changeTableFieldOpt(this,'{$fldname}_opt_num')">
						{include file="Settings/ProcessMaker/actions/TablefieldsOptions.tpl"}
					</select>
				</div>
				{* crmv@239717e *}
			...
			<td colspan="4" nowrap>
				<div id="{$fldname}_adv_options" style="display:none">
					...
					<div style="float:left; padding-right:10px">
						<i class="vteicon md-link" title="{'LBL_SELECT_OPTION_DOTDOTDOT'|getTranslatedString:'com_vtiger_workflow'}" id="{$fldname}_opt_num_editoptions_more" onclick="ActionTaskScript.toggleFieldEditOptions('{$fldname}_opt_num')">more_horiz</i>
					...
					<div style="float:left; width:30%; padding-right:10px">
						<select class="detailedViewTextBox" name="{$fldname}_opt_unit" onchange="if(this.value === 'day') jQuery('#{$fldname}_opt_sla_type_div').show(); else jQuery('#{$fldname}_opt_sla_type_div').hide();">{* crmv@222156 *}
					...
					{* crmv@222156 *}
					<div style="float:left; display: none; width: 20%" id="{$fldname}_opt_sla_type_div">
						<select class="detailedViewTextBox" name="{$fldname}_opt_sla_type">
							<option value="normal">{'LBL_CALENDAR_DAYS'|getTranslatedString:'Processes'}</option>
							<option value="working">{'LBL_WORKING_DAYS'|getTranslatedString:'Processes'}</option>
						</select>
					</div>
					{* crmv@222156e *}
				</div>
...
{elseif $uitype eq 69}
	...
	<div class="{$DIVCLASS}">
		{if $MODULE eq 'Products'}
			...
			<script type="text/javascript" src="{"include/js/multifile.js"|resourcever}"></script>{* crmv@197186 *}
			<script>
		...
		{foreach item=tax key=count from=$TAX_DETAILS}
			...
			<tr style="height:25px">
				...
				<td class="{$DIVCLASS}" width="80%">
					<input type="text" class="detailedViewTextBox" name="{$tax.taxname}" id="{$tax.taxname}" value="{$tax.percentage_fmt}" style="visibility:{$show_value};" {if $smarty.request.enable_editoptions neq "yes"}onBlur="fntaxValidation('{$tax.taxname}')"{/if}> {* crmv@118512 crmv@246980 *}
				</td>
			</tr>
		{/foreach}
	</table>
...
{elseif $uitype eq 70}
	...
	<div class="{$DIVCLASS}">
		...
		<tr>
		<td>
			<input name="{$fldname}" class="detailedViewTextBox input-inline vcenter" tabindex="{$vt_tab}" id="jscal_field_{$fldname}" type="text" maxlength="20" value="{$fldvalue}">
			<i class="vteicon md-link vcenter" id="jscal_trigger_{$fldname}">events</i>
		</td>
		<td style="padding-right:2px;">
		</td>
		</tr>
