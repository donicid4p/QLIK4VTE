...
//CARICAMENTO DI IFRAME IN TAB PER LISTVIEW E IN SETTINGS (panel per detailview in layout editor)
//APPLICA I SOLI FILTRI CON VALORI (1°TIPO)
...
if(vtlib_isModuleActive('QlikIframe') && isPermitted('QlikIframe','')<>"no"){
	...
	$idx='modhomeid';
	if(isset($_REQUEST['qlikiframe_type'])){
		$qlikiframe_type='panels';
		$idx = 'panelid';
	}
	$q_iframe = $adb->pquery("SELECT * 
			FROM qlikiframe_analysis_tab 
			INNER JOIN ".$table_prefix."_".$qlikiframe_type." ON qlikiframe_id = ".$idx."
	...
	if(isset($q_iframe) && $adb->num_rows($q_iframe)>0){ 
		...
		$q_info_qlik =$adb->pquery("SELECT * , ifr.qlikiframeid AS qlikiframe_id_iframe, 
			...
			AND qlikiframe_show_in_tab = 1", array($iframeid,$_REQUEST['qlikiframe_mod'],$_REQUEST['qlikiframe_page']));
		...
		if(isset($q_info_qlik) && $adb->num_rows($q_info_qlik)> 0){
			...
			if(isPermitted("QlikIframe","index",$current_id_iframe)!= "no"){
				...
				$active= $adb->query_result($q_info_qlik,$i,'qlikiframe_active');
				...
				if($active <> 'No'){
					...
					//die("EIFrameLoadCurrentTabAjax ".$newurl);
					//******************************** END	QLIKSENSE *********************************/
					//if the iframe must show (there are credentials, add a element in the array,
					if($riporta){
						$result[]='CORRECT';
						$result[]=array($adb->query_result($q_info_qlik,$i,'qlikiframe_name'),$newurl,  $adb->query_result($q_info_qlik,$i,'qlikiframe_position'), $adb->query_result($q_info_qlik,$i,'qlikiframe_height'));
