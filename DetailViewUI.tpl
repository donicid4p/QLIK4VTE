...
{elseif ($keyfldname eq 'qlikiframe_login_qlik' || $keyfldname eq 'qlikiframe_pwd_qlik') && $isadmin neq 1}
		{include file="FieldHeader.tpl" uitype=$keyid mandatory=$keymandatory label=$label ajaxEditablePerm=$AJAXEDITTABLEPERM}
		<div class="{$DIVCLASS} detailCellInfo" id="fieldCont_{$keyfldid}" {if $AJAXEDITTABLEPERM}onclick="hndMobileClick(this);"{/if}>
	{* qlikiframe - end *}
	{else}
		{include file="FieldHeader.tpl" uitype=$keyid mandatory=$keymandatory label=$label ajaxEditablePerm=$AJAXEDITTABLEPERM}
...
{elseif $keyid eq '19'} <!--TextArea/Description-->
	...
	{elseif $MODULE eq 'HelpDesk' && $keyfldname eq 'comments' && $ID > 0 && $READONLY eq false} {* crmv@240170 *}
		{* crmv@232681 *}
		{* crmv@160733 *}
		<div id="centerbuttons_{$label}" class="requestConfInfoContainer">
			<div class="checkbox">
		...
		{* crmv@160733e *}
		<textarea id="txtbox_{$label}" name="{$keyfldname}" class="detailedViewTextBox sendCommentTextBox" cols="90" rows="5" placeholder="{getTranslatedString('LBL_ADD_COMMENT_PLACEHOLDER', 'HelpDesk')}">{$keyval}</textarea>
		<div class="sendCommentButtonContainer">
			<button type="button" class="crmbutton success sendCommentButton" onclick="VTE.HelpDesk.sendComment('{$MODULE}', '{$ID}');">{getTranslatedString('LBL_SEND')}</button>
		</div>
		{* crmv@232681e *}
	{else}
		{include file="FieldHeader.tpl" uitype=$keyid mandatory=$keymandatory label=$label ajaxEditablePerm=$AJAXEDITTABLEPERM}
		...
		<div id="editarea_{$label}" style="display:none;">
			...
			<tr>
				...
				<td width="80%" style="position:relative">
					...
					{/if}
					<input id="{$keyfldname}_display" name="{$keyfldname}_display" type="text" value="{$fld_displayvalue|escape}" {$fld_style} autocomplete="off"> {* crmv@113776 crmv@234883 *}
					<script type="text/javascript">
						initAutocompleteUG('Users','{$keyfldname}','{$keyfldname}_display',{$JSON->encode($keyoptions.1)},'{$label}',document.DetailView);	{* crmv@52561 crmv@234883 *} 
					</script>
					...
					{if $fld_displaysecondvalue|trim eq ''}
						...
						{assign var=fld_style value='class="detailedViewTextBox"'}
					{/if}
					<input id="assigned_group_id_display" name="assigned_group_id_display" type="text" value="{$fld_displaysecondvalue|escape}" {$fld_style} autocomplete="off"> {* crmv@113776 crmv@234883 *}
					<script type="text/javascript">
						initAutocompleteUG('Groups','assigned_group_id','assigned_group_id_display',{$JSON->encode($keyoptions.2)},'{$label}',document.DetailView);	{* crmv@52561 crmv@234883 *}
					</script>
					<div class="dvtCellInfoImgRx">
